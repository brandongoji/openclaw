...
export const moonshineHandlers: GatewayRequestHandlers = {
  "moonshine.transcribe": async ({ respond, params }) => {
    const p = (params ?? {}) as {
      audioBase64?: unknown;
      model?: unknown;
      language?: unknown;
    ...
    const model = p.model === "base" ? "base" : "tiny";
    ...
    const maxSeconds =
      typeof p.maxSeconds === "number" && Number.isFinite(p.maxSeconds)
        ? Math.max(3, Math.min(30, Math.floor(p.maxSeconds)))
        : 12;
    ...
    if (bytes.length === 0 || bytes.length > 8_000_000) {
      respond(
        false,
        undefined,
        errorShape(ErrorCodes.INVALID_REQUEST, "audio payload too large (max 8MB)"),
      );
      return;
    }
    ...
    try {
      ...
      respond(false, undefined, errorShape(ErrorCodes.UNAVAILABLE, `moonshine transcribe failed: ${message}`));
    } finally {
      await fs.rm(tmpDir, { recursive: true, force: true }).catch(() => undefined);
    }
  },
